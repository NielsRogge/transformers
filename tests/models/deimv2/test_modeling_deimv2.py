...
from transformers import Deimv2Config, DINOv3ViTConfig, is_torch_available, is_vision_available
...
if is_torch_available():
    ...
    from transformers import Deimv2ForObjectDetection, Deimv2ForSegmentation, Deimv2Model
...
class Deimv2ModelTester:
    def __init__(
        ...
        self.encoder_seq_length = (self.min_size // self.patch_size) * (self.max_size // self.patch_size)
    ...
    def get_config(self):
        dinov3_config = DINOv3ViTConfig(
            ...
            out_features=[f"stage{idx}" for idx in range(1, self.num_hidden_layers + 1)],
        )
        return Deimv2Config(
            ...
            backbone_config=dinov3_config,
            backbone_out_features=dinov3_config.out_features,
            backbone=None,
...
class Deimv2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            "image-feature-extraction": Deimv2Model,
            "image-segmentation": Deimv2ForSegmentation,
            "object-detection": Deimv2ForObjectDetection,
        }
        if is_torch_available()
        else {}
    )
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ in ["Deimv2ForObjectDetection", "Deimv2ForSegmentation"]:
                labels = []
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=Deimv2Config, has_text_modality=False)
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if self.is_encoder_decoder:
                ...
                if model_class.__name__ == "Deimv2ForSegmentation":
                    correct_outlen += 3
    ...
    def test_different_timm_backbone(self):
        ...
        for model_class in self.all_model_classes:
            ...
            elif model_class.__name__ == "Deimv2ForSegmentation":
                # Confirm out_indices was propagated to backbone
...
@require_vision
@slow
class Deimv2ModelIntegrationTestsTimmBackbone(unittest.TestCase):
    @cached_property
    def default_image_processor(self):
        return Deimv2ImageProcessor.from_pretrained("facebook/detr-resnet-50") if is_vision_available() else None

    def test_inference_no_head(self):
        model = Deimv2Model.from_pretrained("facebook/detr-resnet-50").to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        model = Deimv2ForObjectDetection.from_pretrained("facebook/detr-resnet-50").to(torch_device)
    ...
    def test_inference_panoptic_segmentation_head(self):
        model = Deimv2ForSegmentation.from_pretrained("facebook/detr-resnet-50-panoptic").to(torch_device)
...
@require_torch
@slow
class Deimv2ModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_image_processor(self):
        return (
            Deimv2ImageProcessor.from_pretrained("facebook/detr-resnet-50", revision="no_timm")
            if is_vision_available()
            else None
        )

    def test_inference_no_head(self):
        model = Deimv2Model.from_pretrained("facebook/detr-resnet-50", revision="no_timm").to(torch_device)
