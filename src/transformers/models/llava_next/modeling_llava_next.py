...
def get_anyres_image_grid_shape(image_size, grid_pinpoints, patch_size):
    ...
    if not isinstance(image_size, (list, tuple)):
        assert isinstance(
            image_size, (torch.Tensor, np.ndarray)
        ), f"image_size invalid type: {type(image_size)} | {image_size}"
    ...
    def _merge_input_ids_with_image_features(
        ...
        * each image has variable length embeddings
        ...
        with torch.no_grad():
            ...
            assert (
                feature_lens.sum() == num_image_features
            ...
            assert (
                total_num_special_image_tokens == num_images
        ...
        if not left_padding:
            ...
            for i, (mask, seq_len) in enumerate(zip(final_attention_mask, seq_lens)):
                ...
                assert torch.all(
                    mask[seq_len:] == 0
                ), f"final 0 mask[{i}]: {seq_len=} {final_attention_mask.size()=} {final_attention_mask.tolist()=}"
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
                ...
                inputs_embeds, attention_mask, position_ids, labels = self._merge_input_ids_with_image_features(
                    image_features, feature_lens, inputs_embeds, input_ids, attention_mask, position_ids, labels=labels
                )
