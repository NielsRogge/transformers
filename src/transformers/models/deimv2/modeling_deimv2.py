...
class Deimv2RepVggBlock(nn.Module):
    ...
    def __init__(self, config: Deimv2Config):
        ...
        self.conv1 = Deimv2ConvNormLayer(config, hidden_channels, hidden_channels, 3, 1, padding=1)
        self.conv2 = Deimv2ConvNormLayer(config, hidden_channels, hidden_channels, 1, 1, padding=0)
        self.activation = nn.Identity() if activation is None else ACT2CLS[activation]()
...
class Deimv2CSPRepLayer(nn.Module):
    ...
    def __init__(self, config: Deimv2Config):
        ...
        self.conv1 = Deimv2ConvNormLayer(config, in_channels, hidden_channels, 1, 1, activation=activation)
        self.conv2 = Deimv2ConvNormLayer(config, in_channels, hidden_channels, 1, 1, activation=activation)
        self.bottlenecks = nn.Sequential(*[Deimv2RepVggBlock(config) for _ in range(num_blocks)])
        if hidden_channels != out_channels:
            self.conv3 = Deimv2ConvNormLayer(config, hidden_channels, out_channels, 1, 1, activation=activation)
        else:
...
class Deimv2MultiheadAttention(nn.Module):
    """
...
class Deimv2HybridEncoder(nn.Module):
    """
    Decoder consisting of a projection layer, a set of `Deimv2Encoder`, a top-down Feature Pyramid Network
    ...
    def __init__(self, config: Deimv2Config):
        ...
        # encoder transformer
        self.encoder = nn.ModuleList([Deimv2Encoder(config) for _ in range(len(self.encode_proj_layers))])

        # top-down FPN
        self.lateral_convs = nn.ModuleList()
        self.fpn_blocks = nn.ModuleList()
        for _ in range(self.num_fpn_stages):
            lateral_conv = Deimv2ConvNormLayer(
                config,
            ...
            fpn_block = Deimv2CSPRepLayer(config)
        ...
        for _ in range(self.num_pan_stages):
            downsample_conv = Deimv2ConvNormLayer(
                config,
                in_channels=self.encoder_hidden_dim,
                out_channels=self.encoder_hidden_dim,
                kernel_size=3,
            ...
            pan_block = Deimv2CSPRepLayer(config)
...
class Deimv2MLPPredictionHead(nn.Module):
    ...
    Origin from https://github.com/lyuwenyu/RT-DETR/blob/94f5e16708329d2f2716426868ec89aa774af016/Deimv2_paddle/ppdet/modeling/transformers/utils.py#L453
...
class Deimv2Model(Deimv2PreTrainedModel):
    def __init__(self, config: Deimv2Config):
        ...
        # https://github.com/lyuwenyu/RT-DETR/blob/94f5e16708329d2f2716426868ec89aa774af016/Deimv2_pytorch/src/zoo/Deimv2/hybrid_encoder.py#L212
        ...
        self.enc_bbox_head = Deimv2MLPPredictionHead(config, config.d_model, config.d_model, 4, num_layers=3)
...
class Deimv2ForObjectDetection(Deimv2PreTrainedModel):
    ...
    def __init__(self, config: Deimv2Config):
        ...
        self.bbox_embed = partial(Deimv2MLPPredictionHead, config, config.d_model, config.d_model, 4, num_layers=3)
