...
class RfDetrWindowedDinov2Config(BackboneConfigMixin, PreTrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`RfDetrWindowedDinov2Model`]. It is used to instantiate an
    RfDetrWindowedDinov2 model according to the specified arguments, defining the model architecture. Instantiating a configuration
    ...
    Args:
            ...
            reshape_hidden_states (`bool`, *optional*, defaults to `True`):
                ...
                seq_len, hidden_size)`.
            num_windows (`int`, *optional*, defaults to 2): <fill_docstring>
            window_block_indexes (`list[int] | None`, *optional*): <fill_docstring>
            gradient_checkpointing (`bool`, *optional*, defaults to `False`): <fill_docstring>

    Example:

    ```python
    >>> from transformers import RfDetrWindowedDinov2Config, RfDetrWindowedDinov2Model

    >>> # Initializing a RfDetrWindowedDinov2 base style configuration
    >>> configuration = RfDetrWindowedDinov2Config()

    >>> # Initializing a model (with random weights) from the base style configuration
    >>> model = RfDetrWindowedDinov2Model(configuration)
...
class RfDetrConfig(PreTrainedConfig):
    ...
    Args:
            backbone_config (`PretrainedConfig` or `dict`, *optional*):
                The configuration of the backbone model. If not provided, will default to `RfDetrViTConfig` with
            ...
            mask_downsample_ratio (`<fill_type>`, *optional*, defaults to 4): <fill_docstring>
            segmentation_bottleneck_ratio (`<fill_type>`, *optional*, defaults to 1): <fill_docstring>
    ...
    >>> # Initializing a LW-DETR AnnaZhang/RfDetr_small_60e_coco style configuration
    >>> configuration = RfDetrConfig()

    >>> # Initializing a model (with random weights) from the AnnaZhang/RfDetr_small_60e_coco style configuration
