...
from .configuration_rf_detr import RfDetrConfig, RfDetrWindowedDinov2Config


class RfDetrWindowedDinov2PatchEmbeddings(nn.Module):
    """
...
class RfDetrWindowedDinov2Embeddings(nn.Module):
    """
    Construct the CLS token, mask token, register tokens, position and patch embeddings.
    """

    def __init__(self, config: RfDetrWindowedDinov2Config) -> None:
        super().__init__()

        self.cls_token = nn.Parameter(torch.randn(1, 1, config.hidden_size))
        self.mask_token = nn.Parameter(torch.zeros(1, config.hidden_size))
        self.register_tokens = nn.Parameter(torch.zeros(1, config.num_register_tokens, config.hidden_size))
        self.patch_embeddings = RfDetrWindowedDinov2PatchEmbeddings(config)
...
class RfDetrWindowedDinov2SelfAttention(nn.Module):
...
class RfDetrWindowedDinov2SelfOutput(nn.Module):
    """
    The residual connection is defined in RfDetrWindowedDinov2Layer instead of here (as is the case with other models), due to the
...
class RfDetrWindowedDinov2Attention(nn.Module):
    def __init__(self, config: RfDetrWindowedDinov2Config):
        super().__init__()
        self.attention = RfDetrWindowedDinov2SelfAttention(config)
        self.output = RfDetrWindowedDinov2SelfOutput(config)
...
class RfDetrWindowedDinov2LayerScale(nn.Module):
...
class RfDetrWindowedDinov2DropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class RfDetrWindowedDinov2MLP(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
...
class RfDetrWindowedDinov2SwiGLUFFN(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        in_features = out_features = config.hidden_size
        hidden_features = int(config.hidden_size * config.mlp_ratio)
...
class RfDetrWindowedDinov2Layer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: RfDetrWindowedDinov2Config) -> None:
        ...
        self.attention = RfDetrWindowedDinov2Attention(config)
        self.layer_scale1 = RfDetrWindowedDinov2LayerScale(config)
        self.drop_path = (
            RfDetrWindowedDinov2DropPath(config.drop_path_rate) if config.drop_path_rate > 0.0 else nn.Identity()
        )

        self.norm2 = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)

        if config.use_swiglu_ffn:
            self.mlp = RfDetrWindowedDinov2SwiGLUFFN(config)
        else:
            self.mlp = RfDetrWindowedDinov2MLP(config)
        self.layer_scale2 = RfDetrWindowedDinov2LayerScale(config)
...
@auto_docstring
class RfDetrWindowedDinov2PreTrainedModel(PreTrainedModel):
    config: RfDetrWindowedDinov2Config
    base_model_prefix = "rf_detr_windowed_dinov2"
    main_input_name = "pixel_values"
    input_modalities = ("image",)
    supports_gradient_checkpointing = True
    _no_split_modules = ["RfDetrWindowedDinov2Layer"]
    _supports_sdpa = True
    _supports_flash_attn = True
    _supports_flex_attn = True
    _supports_attention_backend = True
    _can_record_outputs = {
        "attentions": RfDetrWindowedDinov2SelfAttention,
    }
    ...
    def _init_weights(self, module: nn.Linear | nn.Conv2d | nn.LayerNorm) -> None:
        ...
        elif isinstance(module, RfDetrWindowedDinov2Embeddings):
            init.trunc_normal_(module.position_embeddings, mean=0.0, std=self.config.initializer_range)
            init.trunc_normal_(module.cls_token, mean=0.0, std=self.config.initializer_range)
            init.zeros_(module.mask_token)
            init.zeros_(module.register_tokens)
        elif isinstance(module, RfDetrWindowedDinov2LayerScale):  # noqa: F821
            init.constant_(module.lambda1, self.config.layerscale_value)


@auto_docstring(
    custom_intro="""
    RfDetrWindowedDinov2 backbone, to be used with frameworks like DETR and MaskFormer.
    """
)
class RfDetrWindowedDinov2Backbone(BackboneMixin, RfDetrWindowedDinov2PreTrainedModel):
    ...
    def get_input_embeddings(self) -> RfDetrWindowedDinov2PatchEmbeddings:
...
class RfDetrSamplingLayer(nn.Module):
    def __init__(self, config: RfDetrConfig, channel_size: int, scale: float):
        ...
        if scale == 2.0:
            if channel_size > 512:
                layers.append(RfDetrConvNormLayer(config, channel_size, channel_size // 2, 1, 1, activation="relu"))
...
class RfDetrModel(RfDetrPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("AnnaZhang/RfDetr_small_60e_coco")
        >>> model = DeformableDetrModel.from_pretrained("AnnaZhang/RfDetr_small_60e_coco")
...
class RfDetrForObjectDetection(RfDetrPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("AnnaZhang/RfDetr_small_60e_coco")
        >>> model = RfDetrForObjectDetection.from_pretrained("AnnaZhang/RfDetr_small_60e_coco")
...
__all__ = ["RfDetrForObjectDetection", "RfDetrModel", "RfDetrWindowedDinov2Backbone"]
