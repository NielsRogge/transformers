...
class Sam3LiteTextViTConfig(PreTrainedConfig):
    r"""
    Configuration class for SAM3_LITE_TEXT Vision Encoder (ViT backbone).

    Instantiating a configuration defaults will yield a similar configuration to that of SAM 3
    [facebook/sam3_lite_text](https://huggingface.co/facebook/sam3_lite_text) architecture.
...
class Sam3LiteTextVisionConfig(PreTrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`Sam3LiteTextVisionModel`]. It is used to instantiate a SAM
...
class Sam3LiteTextGeometryEncoderConfig(PreTrainedConfig):
    r"""
    Configuration class for SAM3_LITE_TEXT Geometry Encoder.
...
class Sam3LiteTextDETREncoderConfig(PreTrainedConfig):
    r"""
    Configuration class for SAM3_LITE_TEXT DETR Encoder (vision-text fusion encoder).
...
class Sam3LiteTextDETRDecoderConfig(PreTrainedConfig):
    r"""
    Configuration class for SAM3_LITE_TEXT DETR Decoder (object query decoder).
...
class Sam3LiteTextMaskDecoderConfig(PreTrainedConfig):
    r"""
    Configuration class for SAM3_LITE_TEXT Mask Decoder (pixel-level mask prediction).
...
class Sam3LiteTextConfig(PreTrainedConfig):
    r"""
    Configuration class to store the configuration of a [`Sam3LiteTextModel`].

    Instantiating a configuration defaults will yield a similar configuration to that of SAM 3
    [facebook/sam3_lite_text](https://huggingface.co/facebook/sam3_lite_text) architecture.

    This is the main configuration class that combines all sub-configurations for the SAM3_LITE_TEXT model.

    <Tip>

    SAM3_LITE_TEXT checkpoints with `model_type="sam3_lite_text_video"` are compatible with `Sam3LiteTextModel` since the video variant weights
    ...
    Args:
        vision_config (`dict` or `Sam3LiteTextVisionConfig`, *optional*):
            Configuration for the vision encoder.
        text_config (`dict` or `Sam3LiteTextTextConfig`, *optional*):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(vision_config, dict):
            self.vision_config = Sam3LiteTextVisionConfig(**vision_config)
        else:
        ...
        if isinstance(geometry_encoder_config, dict):
            self.geometry_encoder_config = Sam3LiteTextGeometryEncoderConfig(**geometry_encoder_config)
        else:
        ...
        if isinstance(detr_encoder_config, dict):
            self.detr_encoder_config = Sam3LiteTextDETREncoderConfig(**detr_encoder_config)
        else:
        ...
        if isinstance(detr_decoder_config, dict):
            self.detr_decoder_config = Sam3LiteTextDETRDecoderConfig(**detr_decoder_config)
        else:
        ...
        if isinstance(mask_decoder_config, dict):
            self.mask_decoder_config = Sam3LiteTextMaskDecoderConfig(**mask_decoder_config)
        else:
