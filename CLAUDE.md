...
- `make style`: Apply code style and formatting fixes (runs ruff and other formatters)
- `make fix-repo`: Run all automatic fixes including:
  - Style fixes
  - Modular file conversions (generates `modeling_*.py` from `modular_*.py`)
  - Copy synchronization (updates `# Copied from` blocks)
  - Documentation table of contents updates
  - Dummy file updates
- `make check-repo`: Check repository state without making changes (useful for CI validation)

### When to Run `make fix-repo`

Run `make fix-repo` after:
- Modifying any `modular_*.py` file (to regenerate the corresponding `modeling_*.py`)
- Updating a function that has `# Copied from` references in other files
- Adding or modifying documentation files
- Adding new model classes to ensure auto-mappings are updated